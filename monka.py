...
filename = open('文部科学省 のコピー.txt','r')
outputfile = open('monka.txt','w')
    ...
    if re.search(r'http*',line):
        ...
        if re.search(r'http*',line):
            if is_japanese(line) == False:
                ...
                texts = soup.findAll(id="contents")
                print('---------------',file=outputfile)
                if soup.title != None:
                    print(soup.title.string,file=outputfile)
                """
                if texts == []:
                    texts = soup.findAll(id="kiji")
                elif texts == []:
                    texts = soup.findAll(class_="article-body")
                elif texts == []:
                    texts = soup.findAll(class_="p4 clearfix")
                """
                if texts == []:
